...
このアプリケーションは **SQLDelight** を使用してデータベースを管理しており、スキーマの変更は `.sqm` ファイル（マイグレーションファイル）を通じて履歴管理されています。

- **現在のバージョン確認方法**: `FightersDatabase.Schema.version` プロパティを参照。
- **スキーマ変更手順**:
  1. `database/src/commonMain/sqldelight/...` に連番のマイグレーションファイル（例: `2.sqm`）を作成し、変更SQLを記述。
  2. `FightersDatabase.sq` を最新の構造に更新。
- **自動マイグレーション**: アプリ起動時、`DatabaseDriverFactory` によってスキーマが渡され、必要に応じて自動的に `migrate` 処理が実行されます。
