...
### 4.1. サマリー (Summary) タブ
- **全体統計**: 
  - 期間内の「総大会数」「総対戦数」「合計勝利数」「合計敗北数」を横並びのカード形式で表示。
  - 「総大会数」と「総対戦数」の間には垂直の境界線（Divider）を表示。
- **個人成績詳細（2重円グラフ）**:
  - **内円**: 個人の総合勝率（%）。配色：勝ち＝青、負け＝赤。
  - **外円**: シングルス/チームスそれぞれでの勝利数・敗北数（実数）。
- **チームへの貢献度（2重円グラフ）**:
  - **内円**: チーム（トリオ）全体としての勝率（%）。配色：勝ち＝紫、負け＝橙。
  - **外円**: トリオ戦における自分自身の勝率（%）。配色：勝ち＝青、負け＝赤。チームへの貢献度が視覚的に分かる。
